...
const { selectedGroups, selectedSingles, selectedMember, toggleEntity, fetchFromAPI } =
    ...
    <Disclosure>
      <div class="flex">
        <DisclosureButton class="py-2">
          <h1 class="rounded-xl bg-gray-200 px-3 py-2">{{ group.name }} ({{ group.id }})</h1>
        </DisclosureButton>
        <button @click="toggleEntity('group', props.item)" class="bg-red-200 rounded-xl p-3 ml-4">
          X
        </button>
      </div>
      <DisclosurePanel class="text-gray-500">
        <MemberGroupDisplayItem
          v-for="m in members"
          :key="m.id"
          :member="m"
        ></MemberGroupDisplayItem>
...
// :class="{selectedGroups.value.includes(props.item.id ? props.item.id : props.item) :
'bg-blue-200'}"
